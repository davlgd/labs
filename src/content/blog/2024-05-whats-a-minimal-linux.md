...
```bash
qemu-system-x86_64 -kernel arch/x86/boot/bzImage -initrd initramfs.cpio.gz --enable-kvm -cpu host -append "console=ttyS0 panic=1" -nographic -no-reboot
```

You should see the kernel starting, the `initramfs` compressed file is mounted, the `init` script found and started. Then, the "Hello, world!" message is printed before the system stops (because there is nothing more to do).
...
You should see the system booting and get a shell prompt. You can now use some commands:
...
It's your first minimal Linux system, have fun with it! 🎉

_PS: You'll find the full script from this guide here._
